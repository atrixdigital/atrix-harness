...
| Any UI — app, site, page, component | **A direction chosen for this brief** | `choosing-a-design-direction`, then `building-ui`, `layout-and-spacing`, `motion-and-interaction`, `verifying-ui-visually` |
...
The escape is process, not a better adjective. **Pitch the options to the human before writing
code** — `choosing-a-design-direction` — and pin the winner to `DESIGN.md` so the next session
builds from the same decision instead of re-deriving a different one. `frontend-design` carries the
generation process behind those pitches.
